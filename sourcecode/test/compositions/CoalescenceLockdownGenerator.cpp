...
<<
std::endl;
ss << "    measureIndex = " << insideMeasureIndex << ";" <<
std::endl;
ss << "    noteIndex = " << insideNoteIndex << ";" <<
std::endl;
...
ss << "" <<
std::endl;
ss << "    partIndex = VIOLIN_1;" <<
std::endl;
ss << "    expectedMidiNumber = " << insideMidiNum << ";" <<
std::endl;
ss << "    isAccentExpected = " << insideIsAccent << ";" <<
std::endl;
ss << "    mxNote = score.parts.at( partIndex ).measures.at( measureIndex ).staves.at( 0 ).voices.at( 0 ).notes.at( noteIndex );"
<<
std::endl;
ss << "    atom = scriabin::Atom{ mxNote.pitchData };" <<
std::endl;
ss << "" <<
std::endl;
ss << "    CHECK( atom.getMidiNote() == expectedMidiNumber );" <<
std::endl;
ss << "    CHECK( isAccented( mxNote ) == isAccentExpected );" <<
std::endl;
